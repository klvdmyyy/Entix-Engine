...
namespace Entix
{
    ...
    public:
        ...
        EX_FORCE_INLINE
        inline void
        SetAssetsDirectory(const std::filesystem::path& path) noexcept
        {
            m_assetDir = path;
        }
        ...
        //
        // Different filepath produce different hashes in ResourceId. Just using
        // '/' instead of '\' is already can cause a lot of errors.
        //
        // TODO: We need to always replace all '\' with '/' symbol
        EX_FORCE_INLINE
        inline std::filesystem::path
        GetAsset(const std::filesystem::path& path) noexcept
        {
        ...
        {
            EX_LOG(Trace, LogCategory::Resource,
                   "Registering new resource loader: {}", TypeName<T>());
        ...
        {
            this->RegisterLoader<T>(
                Scope<ResourceLoader>(new T(std::forward<Args>(args)...)));
        }

        template <std::derived_from<Resource> T,
                  std::derived_from<ResourceLoader> Loader>
        [[nodiscard]]
        ...
        void UnloadAll()
        {
            EX_ASSERT(m_loaders.contains(typeid(T).hash_code()),
                      "Using unregistered loader for resource. Loader: '{}'",
                      TypeName<T>());
    ...
    private:
        ...
        template <std::derived_from<Resource> T,
                  std::derived_from<ResourceLoader> Loader>
        ResourceHandle<T> LoadInternal(const ResourceId& id,
                                       [[maybe_unused]] LoadMode mode)
        {
            ...
            EX_ASSERT_FMT(
                m_loaders.contains(typeid(Loader).hash_code()),
                "Using unregistered loader for resource. Loader: '{}'",
                TypeName<Loader>());
            ...
            {
                EX_LOG(Resources, Trace, "Loading '{}' using {}",
                ...
                if (mode == LoadMode::Async)
                {
                    EX_LOG(Resources, Error,
                           "Async resource loading is unimplemented for now. "
                           "Resource loading are skipped. Handle object of "
                           "resource can throw the assertion!");
                    this->LoadEnd();
                    return ResourceHandle<T>();
                } else
                {
                    Scope<ResourceLoader>& loader =
                        m_loaders.at(typeid(Loader).hash_code());
            ...
            {
                EX_LOG(Resources, Debug,
                       "'{}' is already loaded. Just returning it!",
                       std::filesystem::relative((std::filesystem::path)(id),
                                                 m_assetDir)
                           .string());
            }
            ...
            EX_ASSERT_FMT(
                res, "Failed to cast resource provided by loader to '{}' type!",
                TypeName<T>());
        ...
        std::unordered_map<ResourceId, Scope<Resource>, ResourceId::Hasher>
            m_resources;
